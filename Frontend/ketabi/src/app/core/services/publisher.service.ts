...
import {
  ...
  UpdateBookRequest,
} from '../../features/publishers/models/book.model';
import {
  PublisherOrdersResponse,
  UpdatePublisherOrderRequest,
  UpdatePublisherOrderResponse,
...
export class PublisherService {
  ...
  private profileUrl = API_ENDPOINTS.profile;
  ...
  getPublishedBooks(
    ...
    page: number = 1,
    limit: number = 10
  ): Observable<PublisherBooksResponse> {
    const params = {
      page: Math.max(1, page || 1),
      limit: Math.max(5, limit || 10),
    };
    return this.http.get<PublisherBooksResponse>(`${this.publishersUrl}/${publisherId}/books`, {
      params,
    });
  }
  ...
  getPublisherBook(bookId: string): Observable<PublisherBook> {
    ...
    return this.http
      .get<any>(`${this.booksUrl}/Get-Book/${bookId}`, { headers })
      .pipe(map((response) => response?.data?.book ?? response?.data ?? response));
  }
  ...
  getPublisherOrders(
    ...
    page: number = 1,
    limit: number = 10
  ): Observable<PublisherOrdersResponse> {
    const params = {
      page: Math.max(1, page || 1),
      limit: Math.max(5, limit || 10),
    };
  ...
  updatePublisherOrder(
    ...
    data: UpdatePublisherOrderRequest
  ): Observable<UpdatePublisherOrderResponse> {
    const headers = this.getAuthHeaders();
    return this.http.patch<UpdatePublisherOrderResponse>(
      `${this.publishersUrl}/${publisherOrderId}`,
      data,
      { headers }
    );
  }
