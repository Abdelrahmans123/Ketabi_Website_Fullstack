...
export class CartService {
  ...
  addItem(book: Book, quantity: number = 1, type: 'physical' | 'ebook') {
    ...
    if (existingItem) {
      ...
      type === 'physical' ? existingItem.quantity = Math.min(existingItem.quantity + quantity, existingItem.stock) : existingItem.quantity = 1;
    } else {
      cart.items.push({
        bookId: book._id,
        name: book.name,
  ...
  private updateTotals(cart: Cart) {
    cart.total = cart.items.reduce((sum, item) => sum + item.price * (item.type === 'ebook' ? 0.45 : 1) * item.quantity * (1 - item.discount / 100), 0);
    this.cartSubject.next(cart);
    this.saveCart();
  }
